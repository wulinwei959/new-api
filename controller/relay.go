...
func Relay(c *gin.Context, relayFormat types.RelayFormat) {
	...
	if newAPIError != nil {
		gopool.Go(func() {
			...
			// 服务端错误(5xx)或限流(429)累加连续失败计数，达到阈值即触发渠道冷却
			if newAPIError != nil && (newAPIError.StatusCode >= 500 || newAPIError.StatusCode == http.StatusTooManyRequests) && relayInfo.ChannelId > 0 {
				service.RecordConsecutiveFailure(relayInfo.ChannelId)
			}
		})
	}
}
